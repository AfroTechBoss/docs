...
description: "Every revert error produced by the IFÁ Labs oracle contract and integration patterns — causes, selectors, and resolution steps."
---
...
See [Supported Assets](/introduction/supported-assets) for the full list of valid asset IDs.
...
**Resolution:** Verify the asset ID using the diagnosis script in [Common Integration Errors](/troubleshooting/common-integration-errors#exists--false).
...
3. Report it immediately to [security@ifalabs.com](mailto:security@ifalabs.com) if the asset exists but the price is zero
...
**Resolution:** Identify which asset ID in your batch returned `exists = false` and debug that specific ID using the diagnosis steps in [Common Integration Errors](/troubleshooting/common-integration-errors#exists--false).
