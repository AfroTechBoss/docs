...
Answers to the questions developers and protocol teams ask most often. If your question is not covered here, reach out via any [support channel](/support-and-community/get-help).
...
<AccordionGroup>
  <Accordion title="What is IFÁ Labs?">
    ...
    See [What is IFÁ Labs?](/introduction/what-is-ifa-labs) for the full overview.
  </Accordion>
  ...
  <Accordion title="How is IFÁ Labs different from Chainlink or Pyth?">
    ...
    Many protocols use both: IFÁ Labs for stablecoin feeds, Chainlink or Pyth for volatile assets. See [Key Differentiators](/introduction/key-differentiators) for the full comparison.
  </Accordion>
  ...
  <Accordion title="Which chains does IFÁ Labs support?">
    ...
    See [Supported Chains & Roadmap](/introduction/supported-chains-and-roadmap) for full details and timelines.
  </Accordion>
  ...
  <Accordion title="Which stablecoins does IFÁ Labs support?">
    ...
    New assets are added on a rolling basis. See [Supported Assets](/reference/supported-assets) for the complete list including asset IDs.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="How do I read a price from IFÁ Labs in Solidity?">
    ...
    See [Read Latest Price](/integration-guides/evm-smart-contracts/read-latest-price) for full examples including batch reads and derived pairs.
  </Accordion>
  ...
  <Accordion title="What are asset IDs and how are they generated?">
    ...
    You can independently verify any asset ID by computing the hash yourself and comparing against the published value. See [Working with Asset IDs](/integration-guides/evm-smart-contracts/working-with-asset-ids) for full details.
  </Accordion>
  ...
  <Accordion title="How do I convert the raw price to a human-readable value?">
    ...
    Always read `decimal` dynamically rather than hardcoding `18` — future assets may use different precision. See [Decimal Precision & Formatting](/core-concepts/decimal-precision-and-formatting) for full conversion guidance and edge cases.
  </Accordion>
  ...
  <Accordion title="What staleness threshold should I use?">
    ...
    For production deployments, run the cadence analysis script in [Price Appears Stale](/troubleshooting/price-appears-stale) to calibrate thresholds from real on-chain data rather than documentation estimates.
  </Accordion>
  ...
  <Accordion title="Can I get prices for multiple assets in one call?">
    ...
    One call is significantly cheaper than N consecutive `getAssetInfo` calls. Always use batch reads when you need more than one price in the same transaction. See [Gas Optimization Tips](/integration-guides/evm-smart-contracts/gas-optimization-tips).
  </Accordion>
  ...
  <Accordion title="How do I price CNGN against USDT — or any asset against another?">
    ...
    Any combination of the six supported assets is computable as a derived pair. See [Function Reference](/onchain-reference/smart-contracts/function-reference) for the full derived pair API.
  </Accordion>
  ...
  <Accordion title="Can I use IFÁ Labs from JavaScript without writing Solidity?">
    ...
    See [Get Your First Price](/quickstart/get-your-first-price) for full off-chain examples in JavaScript, Python, and via Basescan.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Have the IFÁ Labs contracts been audited?">
    ...
    The full audit summary is at [AdForensics Audit Summary](/security-and-audits/adforensics-audit-summary). The complete report is available on request from [ifalabstudio@gmail.com](mailto:ifalabstudio@gmail.com) and in the [GitHub repository](https://github.com/IFA-Labs/oracle_contract).
  </Accordion>
  ...
  <Accordion title="What happens if a relayer submits a wrong price?">
    ...
    See [Data Integrity Mechanisms](/security-and-audits/data-integrity-mechanisms) for the full seven-layer defense architecture.
  </Accordion>
  ...
  <Accordion title="How do I verify IFÁ Labs prices independently?">
    ...
    Full instructions for all five methods: [Verification Proofs](/security-and-audits/verification-proofs).
  </Accordion>

  <Accordion title="How do I report a security vulnerability?">
    Email [security@ifalabs.com](mailto:security@ifalabs.com) with a description of the issue, reproduction steps, and your assessment of severity. Do not disclose security issues publicly before coordinating with IFÁ Labs.

    We acknowledge all reports within 48 hours. See [Report a Vulnerability](/security-and-audits/report-a-vulnerability) for the full responsible disclosure policy.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="What is the IFÁ Labs MCP Server?">
    ...
    See [What is the IFÁ Labs MCP Server?](/integration-guides/mcp-server/what-is-the-ifa-labs-mcp-server) for full details.
  </Accordion>
  ...
  <Accordion title="How do I install the MCP server?">
    ...
    Full installation instructions for every supported client: [MCP Server Installation & Setup](/integration-guides/mcp-server/installation-and-setup).
  </Accordion>
  ...
  <Accordion title="Which MCP clients does IFÁ Labs support?">
    ...
    See [MCP Client Compatibility](/integration-guides/mcp-server/mcp-client-compatibility) for client-specific configuration and known limitations.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Why is my price feed showing as stale?">
    ...
    Set `MAX_PRICE_AGE` to at least 1.5× the heartbeat interval for the asset. Then run the cadence analysis script in [Price Appears Stale](/troubleshooting/price-appears-stale) to calibrate from real on-chain data.
  ...
  <Accordion title="Are asset IDs the same across all networks?">
    ...
    The only thing that changes between networks is the oracle contract address. See [Contract Addresses](/onchain-reference/smart-contracts/contract-addresses) for the address per network.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="How do I stay updated on new deployments and assets?">
    ...
    The [Changelog](/reference/changelog) is updated for every significant release.
  </Accordion>
  ...
  <Accordion title="Can I contribute to IFÁ Labs?">
    ...
    See [GitHub & Bug Reports](/support-and-community/github-and-bug-reports) for the full contribution workflow, repository structure, and contribution areas.
  </Accordion>
  ...
  <Accordion title="Is there a bug bounty program?">
    ...
    See [Report a Vulnerability](/security-and-audits/report-a-vulnerability) for the current disclosure policy and contact channels.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Get Help" icon="message" href="/support-and-community/get-help">
