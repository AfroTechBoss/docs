...
description: "Fetch live stablecoin prices from the IFÁ Labs oracle in your smart contracts — single assets, batch reads, and derived pairs."
---
...
**Using the price without a staleness check.** A returned price with `exists = true` is not automatically fresh. Always pair your read with a `lastUpdateTime` check before using the value in any logic with financial consequences. See [Verify Price Integrity](/quickstart/verify-price-integrity).
...
<CardGroup cols={2}>
  ...
  <Card title="Gas Optimization Tips" icon="gauge" href="/integration-guides/evm-smart-contracts/gas-optimization-tips">
